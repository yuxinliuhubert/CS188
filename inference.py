...
def constructBayesNet(gameState: hunters.GameState):
    ...
    maxManhattanDistance = X_RANGE + Y_RANGE - 2
    ...
    variableDomainsDict = {
        ...
        OBS0: list(range(maxManhattanDistance + MAX_NOISE + 1)),  # OBS0 domain
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            # Join all factors containing the variable
            factorsToJoin = [f for f in factors if variable in f.variablesSet()]
            if factorsToJoin:
                _, joinedFactor = joinFactorsByVariable(factorsToJoin, variable)

                # Eliminate the variable from the joined factor, if appropriate
                if len(joinedFactor.unconditionedVariables()) > 1 or variable not in joinedFactor.unconditionedVariables():
                    ...
                    # Discard the factor if it only has one unconditioned variable
                    factors = [f for f in factors if variable not in f.variablesSet()]

        finalFactor = normalize(joinFactors(factors))
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            new_position_distribution = self.getPositionDistribution(gameState, oldPos)

            for new_position, new_probability in  new_position_distribution.items():
                new_beliefs[new_position] += self.beliefs[oldPos] * new_probability
